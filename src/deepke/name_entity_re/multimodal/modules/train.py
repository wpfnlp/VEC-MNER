...
class Trainer(object):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    attention_mask, labels, labels_x, logits, loss = self._step(batch, mode="train")
                    avg_loss += loss.detach().cpu().item()

                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip_grad_norm)  # 梯度裁剪
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    attention_mask, labels, labels_x, logits, loss = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()

                    if isinstance(logits, torch.Tensor):    
                        logits = logits.argmax(-1).detach().cpu().numpy()  # batch, seq, 1
                    label_ids = labels_x.detach().cpu().numpy()
    ...
    def _step(self, batch, mode="train"):
        input_ids, token_type_ids, attention_mask, labels, labels_x, images, aux_imgs, rcnn_imgs = batch
        ...
        return attention_mask, labels, labels_x, logits, loss
    ...
    def multiModal_before_train(self):
        ...
        # params['params'] = []
        # for name, param in self.model.named_parameters():
        #     if ('text' in name) and ('senet' not in name):
        #         params['params'].append(param)
        # parameters.append(params)
        # # senet lr
        # parameters = []
        # params = {'lr':5e-3, 'weight_decay':1e-2}
        # params['params'] = []
        # for name, param in self.model.named_parameters():
        #     if 'senet' in name:
        #         params['params'].append(param)
        ...
        linear_param_optimizer = list(self.model.fc.named_parameters())
        optimizer_grouped_parameters = [
            ...
            'weight_decay': self.args.weight_decay, 'lr': self.args.crf_lr},
