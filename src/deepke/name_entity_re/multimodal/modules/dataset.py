...
class MMPNERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, word in enumerate(word_list):
            ...
            for m in range(len(token)):
                if m == 0:
                    ...
                    labels_x.append(self.label_mapping[label])
                else:
                    labels.append(self.label_mapping[label])
                    labels_x.append(self.label_mapping["X"])
        if len(tokens) >= self.max_seq - 1:
            tokens = tokens[0:(self.max_seq - 2)]
            labels = labels[0:(self.max_seq - 2)]
            labels_x = labels_x[0:(self.max_seq - 2)]
        ...
        labels_x = [self.label_mapping["[CLS]"]] + labels_x + [self.label_mapping["[SEP]"]] + [self.ignore_idx]*(self.max_seq-len(labels_x)-2)
        ...
        if self.img_path is not None:
            ...
            if self.aux_img_path is not None:
                ...
                if self.rcnn_img_path is not None:
                    ...
                    return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_x), image, aux_imgs, rcnn_imgs

                return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_x), image, aux_imgs

        assert len(input_ids) == len(token_type_ids) == len(attention_mask) == len(labels) == len(labels_x)
        return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_x)